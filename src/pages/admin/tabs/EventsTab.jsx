...
import { Search, Plus, Trash2, Edit3, Save, AlertCircle, CalendarDays } from 'lucide-react'
...
export default function EventsTab() {
  ...
  const [form, setForm] = useState({ title: '', description: '', date: '', time: '', location: '', image: '', category: 'event', capacity: '', contactEmail: '', contactPhone: '', isPublished: true })
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true)
    try { const data = await eventsAPI.getAll(); setItems(data.items || []) } catch { /* empty */ }
    setLoading(false)
  }, [])
  ...
  const resetForm = () => {
    setForm({ title: '', description: '', date: '', time: '', location: '', image: '', category: 'event', capacity: '', contactEmail: '', contactPhone: '', isPublished: true })
    setEditing(null); setShowForm(false); setError('')
  }

  const handleEdit = (item) => {
    setForm({ title: item.title || '', description: item.description || '', date: item.date || '', time: item.time || '', location: item.location || '', image: item.image || '', category: item.category || 'event', capacity: item.capacity || '', contactEmail: item.contactEmail || '', contactPhone: item.contactPhone || '', isPublished: item.isPublished !== false })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        ...
        const data = await eventsAPI.add(form)
  ...
  const handleDelete = async (id) => {
    setDeleteId(id)
    try { await eventsAPI.delete(id); setItems((prev) => prev.filter((i) => i._id !== id)) } catch { /* empty */ }
    setDeleteId(null)
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-boma-charcoal">Events & Functions</h1>
          <p className="text-sm text-boma-charcoal/60 mt-1">{items.length} event{items.length !== 1 ? 's' : ''}</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <input type="text" required value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="Event name" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Location</label>
                <input type="text" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="Venue" />
              </div>
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Date</label>
              ...
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Time</label>
                <input type="text" value={form.time} onChange={(e) => setForm({ ...form, time: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="e.g. 7:00 PM" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Capacity</label>
                <input type="text" value={form.capacity} onChange={(e) => setForm({ ...form, capacity: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="e.g. 200 guests" />
              </div>
              <div className="md:col-span-2">
                ...
                <textarea rows={4} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust resize-y" placeholder="Event details..." />
              </div>
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Contact Email</label>
                <input type="email" value={form.contactEmail} onChange={(e) => setForm({ ...form, contactEmail: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="events@boma.com" />
              </div>
              <div>
                <label className="block text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider mb-1">Contact Phone</label>
                <input type="tel" value={form.contactPhone} onChange={(e) => setForm({ ...form, contactPhone: e.target.value })}
                  className="w-full px-3 py-2 border border-boma-charcoal/20 rounded-lg text-sm focus:outline-none focus:border-boma-rust" placeholder="+263..." />
              </div>
              <div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-boma-charcoal/10 bg-gray-50/50">
                  <th className="text-left text-xs font-medium text-boma-charcoal/60 uppercase tracking-wider px-5 py-3">Event</th>
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item._id} className="border-b border-boma-charcoal/5 last:border-0 hover:bg-gray-50/50 transition-colors">
                    <td className="px-5 py-4">
                      ...
                      {item.location && <p className="text-xs text-boma-charcoal/50">{item.location}</p>}
                    </td>
                    <td className="px-5 py-4">
                    ...
                    <td className="px-5 py-4 text-sm text-boma-charcoal/70">{item.date || formatDate(item.createdAt)}</td>
                    <td className="px-5 py-4">
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={5} className="px-5 py-12 text-center text-sm text-boma-charcoal/50">No events found</td></tr>
                )}
              </tbody>
            </table>
          </div>
