...
function HeroBanner() {
  ...
  return (
    <VideoHero
      ...
      title={data.title}
      subtitle="From legendary dining shows to sundowners overlooking the Zambezi — discover culinary experiences that celebrate Africa."
...
function DiningCard({ option, index }) {
    ...
    <section ref={ref} className="section-padding py-20 md:py-28 bg-boma-off-white">
      <div className="max-w-7xl mx-auto">
        <SectionHeading
          ...
          subtitle="Explore our world-class restaurants, bars and dining shows"
        />
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
          {data.options.map((option, i) => (
            <DiningCard key={option.slug} option={option} index={i} />
          ))}
        </div>
      </div>
    </section>
    ...
    <section ref={ref} className="section-padding py-20 md:py-28 bg-boma-charcoal">
      <div className="max-w-4xl mx-auto">
        <SectionHeading
          ...
          subtitle="Dining information and dietary requirements"
          light
        />
...
export default function WineAndDine() {
  const data = vfscData.wineAndDine
